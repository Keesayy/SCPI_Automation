...
def Gpid_devices_open():
	...
	signal_source = rm.open_resource('TCPIP::192.168.1.50::INSTR')
...
def Sweep_freq(power_meter, signal_source, freq_start : float, freq_stop : float, nb_points : int, dwel : float, amplitude : float):
	...
	# Save the workbook
	new_workbook_name = Excel_name('MI-3111-SW_F', 0, freq_start, freq_stop, nb_points, dwel, amplitude)	
...
def Sweep_amp(power_meter, signal_source, amp_start : float, amp_stop : float, nb_points : int, dwel : float, freq_list = [0.1, 2, 6, 12, 18]):
	freq_list = list(map(Hz_to_GHz, freq_list))
	...
	for k in range(len(freq_list)):
		print('\nFrequence : ' + str(freq_list[k]))

		amp = amp_start
		signal_source.write('FREQ:CW %f' % freq_list[k])
	...
	# Save the workbook
	new_workbook_name = Excel_name('MI-3111-SW_A', 0, amp_start, amp_stop, nb_points, dwel, freq_list)	
	excel.save(new_workbook_name)
	return excel, new_workbook_name
